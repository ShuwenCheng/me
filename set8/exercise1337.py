...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        pets_list = pet_filter(letter)
        pets_count = len(pets_list)
        if pets_count > max_pets_count:
            max_pets_count = pets_count
...
def make_filler_text_dictionary() -> Dict:
    ...
    for word_length in range(3, 8):
        word_list = []
        for _ in range(4):  # Generate 4 words for each word length
            response = requests.get(url + str(word_length))
            if response.status_code == 200:
                word_list.append(response.text.strip())
